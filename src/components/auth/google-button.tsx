...
export function GoogleButton({ label }: { label: string }) {
  ...
  return (
    <button
      ...
      onClick={() => {
        setLoading(true);
        // UI-only placeholder; real OAuth wiring goes here later.
        setTimeout(() => setLoading(false), 1200);
      }}
      disabled={loading}
      className="w-full inline-flex items-center justify-center gap-3 h-11 rounded-md border border-[#E3E8EE] bg-white hover:bg-[#F6F9FC] transition-colors text-sm font-medium text-[#0A2540] disabled:opacity-60"
    >
      {loading ? (
